          ...
          builder: (context) {
            if (isBusy) {
              return SizedBox.square(
                child: FittedBox(
                  child: AppLoader(padding: 10, color: labelColor),
                ),
              );
            }
...
class AppBackButton extends StatelessWidget {
  ...
  final VoidCallback? onPressed;
  const AppBackButton({super.key, this.alignment, this.onPressed});
  ...
  Widget build(BuildContext context) {
    return IconButton(
      ...
      icon: Icon(Icons.arrow_back_ios_new_rounded, size: 14.r),
    );
  }
}
